...
import {HomePageText, HomePageTextResponse, SqlLanguage} from "../src/app/dal/models";


export function handleHomeRoutes(req: Request, res: Response) {
  let result: HomePageTextResponse = {items: []};
  ...
  function executeStatement() {
    ...
    request.on('row', function (columns: any[]) {
      let obj: HomePageText = {Text: '', ID: -1, TimeStamp: new Date()}
      columns.forEach(i => {
